...
def process_weapon_detections(results_hammer, results_knife, results_gun, results_bat, results_axe, confidence_threshold):
    ...
    # Process bat detections
    if results_bat[0].boxes:
        bat_boxes = results_bat[0].boxes
        bat_confidences = bat_boxes.conf.cpu().numpy()
        filtered_bat_indices = np.where(bat_confidences >= 0.6)[0]
        weapon_boxes.extend(bat_boxes.xyxy.cpu().numpy()[filtered_bat_indices])
        weapon_classes.extend(["Bat"] * len(filtered_bat_indices))  # Label as "Bat"
        weapon_confidences.extend(bat_confidences[filtered_bat_indices])

    # Process axe detections
    if results_axe[0].boxes:
        axe_boxes = results_axe[0].boxes
        axe_confidences = axe_boxes.conf.cpu().numpy()
        filtered_axe_indices = np.where(axe_confidences >= 0.6)[0]
        weapon_boxes.extend(axe_boxes.xyxy.cpu().numpy()[filtered_axe_indices])
        weapon_classes.extend(["Axe"] * len(filtered_axe_indices))  # Label as "Axe"
        weapon_confidences.extend(axe_confidences[filtered_axe_indices])
