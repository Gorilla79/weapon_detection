...
while True:
    ...
    for human_box, human_id in zip(boxes_human, ids_human):
        ...
        for weapon_box in boxes_weapon:
            ...
            distance = np.linalg.norm(np.array(human_center) - np.array([(weapon_box[0] + weapon_box[2]) / 2, (weapon_box[1] + weapon_box[3]) / 2]))
    ...
    if interested_object_id is not None:
        for human_box, human_id in zip(boxes_human, ids_human):
            if human_id == interested_object_id:
                ...
                label_coords = f"X:{center_x} Y:{center_y} Z:{depth:.2f}m"
                ...
                cv2.putText(im0, label_coords, (x1, max(y1 - 10, 0)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
